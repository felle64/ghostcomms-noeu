...
export type Msg = { id: string; text: string; mine?: boolean; delivered?: boolean; ts?: number; system?: boolean }
...
export function useThread({ self, peer }: { self: string; peer: string }) {
  ...
  function connect(initial=false) {
    ...
    ws.onmessage = async (ev) => {
      ...
      // normal incoming/backlog (from could be '(offline)')
    ...
    ws.onclose = (ev) => {
      ...
      if (reconnectAttemptsRef.current >= maxReconnectAttempts) {
        setStatus('offline'); setErr('Connection failed. Refresh to retry.'); return
      }
  ...
  async function send(e: React.FormEvent) {
    ...
    const ct = await encryptFor(peer, enc(text))
  ...
  async function clearThread() {
    ...
    const alsoNotify = confirm('Also notify the other device that you cleared this chat?')
    await dbClearThread(peer)
    setItems([])
    if (alsoNotify && wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
