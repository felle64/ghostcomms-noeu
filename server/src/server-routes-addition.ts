...
app.get('/linking/provision/:deviceId', async (req, reply) => {
...
app.post('/history/archive', async (req, reply) => {
  try {
    await (req as any).jwtVerify()
  } catch {
    return reply.status(401).send({ error: 'unauthorized' })
  ...
  const encryptedData = new Uint8Array(await req.raw())
...
app.get('/history/download/:archiveKey', async (req, reply) => {
  const archiveKey = (req.params as any).archiveKey as string
